...
describe "Mongar" do
  before(:all) do
    ...
    mongo.connection!.database.drop
  end
  describe "run" do
    before do
    ...
    it "should add, delete, and update items properly" do
      ...
      @mongar.run
      @collection.find({ :name => 'test.com' }).first.should include({'name' => 'test.com', 'client_id' => 1})
      
      # updates
      sleep 1
      domain.client_id = 2
      @mongar.run
      @collection.find({ :name => 'test.com' }).first.should include({'name' => 'test.com', 'client_id' => 2})

      # deletes
      sleep 1
      domain.destroy
      @mongar.run
      @collection.find({:name => "test.com"}).first.should be_nil
      
      collection = Mongar::Mongo.databases[:default].db['stati']
      collection.find({ :collection_name => 'domains' }).first.should_not be_nil
    end
    ...
    it "should handle an item that was added, updated, and deleted between runs" do
      ...
      @collection.find({:name => "test1.com"}).first.should be_nil
    end
  end
end
